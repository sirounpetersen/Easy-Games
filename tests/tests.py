...
class BasicTests(unittest.TestCase):
    ...
    def test_platform_page(self): # Checks if the page opens functionally
        ...
        self.assertEqual(response.status_code, 200)
    def test_platformGames_page(self): # Checks if the page opens PC functionally
        platformName = "PC"
        games = platform_games(platformName)
        response = self.app.post('/platformGames', data=dict(console = platformName, games=games))
        self.assertEqual(response.status_code, 200)
    def test_search_page(self): # Checks if the page opens Valorant functionally
        name = "valorant"
        detail, image, rate, platform, platformRate,website = search(name)
        response = self.app.post("/search", data=dict(name=name.upper(), detail=cleanhtml(detail), image=image, rate=rate, website=website, both=zip(platform,platformRate)))
        self.assertEqual(response.status_code, 200)
